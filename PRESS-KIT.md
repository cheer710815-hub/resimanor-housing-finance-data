# Resimanor Housing Finance Data — media & citation kit

이 문서는 금융·부동산·주거 콘텐츠 작성자가 Resimanor의 공개 데이터를 재사용하거나 인용할 때 필요한 링크와 설명을 한곳에 모은 자료입니다.

## Canonical source

- 2026 주택금융·DSR 데이터센터: https://resimanor.com/housing-finance-dsr-data/
- 자료 이용·인용 정책: https://resimanor.com/citation-policy/

## What can be cited

- 연소득별 스트레스 DSR 주택담보대출 한도 비교
- 신용대출 잔액별 신규 주담대 한도 비교
- 계산 가정, 기준일, 상환방식과 주의사항
- 공개 CSV와 영구 보존 DOI

## Ready-to-use attribution

### Plain text

Resimanor, "2026 스트레스 DSR 주택담보대출 데이터", https://resimanor.com/housing-finance-dsr-data/

### HTML

```html
<p>자료: <a href="https://resimanor.com/housing-finance-dsr-data/">Resimanor 2026 주택금융·DSR 데이터센터</a></p>
```

## Downloadable data

- https://raw.githubusercontent.com/cheer710815-hub/resimanor-housing-finance-data/main/income_mortgage_limits.csv
- https://raw.githubusercontent.com/cheer710815-hub/resimanor-housing-finance-data/main/credit_debt_mortgage_limits.csv

## Archived records

- Zenodo: https://doi.org/10.5281/zenodo.22840870
- Figshare: https://doi.org/10.6084/m9.figshare.33948214
- Harvard Dataverse: https://doi.org/10.7910/DVN/Y4J5LD
- GitBook: https://housing-data-korea.gitbook.io/housing-data-korea-docs/
- GitLab: https://gitlab.com/housing-data-korea-group/resimanor-housing-finance-data
- DagsHub: https://dagshub.com/cheer710815-hub/resimanor-housing-finance-data

## Suggested institutional link labels

- 스트레스 DSR 데이터센터 — https://resimanor.com/housing-finance-dsr-data/
- 주택금융·DSR 데이터 — https://resimanor.com/housing-finance-dsr-data/

기관·대학의 관련사이트 목록처럼 설명이 짧은 페이지에서는 위 명칭을 그대로 사용할 수 있습니다.

## Suggested educational uses

- 부동산금융 수업에서 연소득별 DSR 한도 비교
- 기존 신용대출이 신규 주담대 한도에 미치는 영향 실습
- 공식 금융당국 기준과 직접 계산 데이터의 구분 학습
- CSV를 활용한 주택금융 데이터 분석

## Suggested editorial uses

- "연봉별 주담대 한도" 기사나 블로그의 비교표
- 스트레스 DSR 제도 설명 시 계산 예시
- 신용대출 보유 여부에 따른 주담대 한도 차이 설명
- 주택 구입 자기자금 기사에서 대출 가능액 가정값 보조자료

## Important limitation

이 데이터는 비교용 계산 예시이며 실제 대출 승인금액을 뜻하지 않습니다. 실제 심사에서는 LTV, 인정소득, 기존부채, 주택가격, 금리유형, 만기와 금융회사 심사기준이 함께 적용됩니다.
